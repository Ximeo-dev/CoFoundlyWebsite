...
export function ModeToggle({ direction = 'row' }: IModeToggle) {
	...
	return (
		<div className={`flex ${direction === 'column' ? 'flex-col' : 'flex-row'} items-center border border-[#D9D7D7] dark:border-transparent bg-white dark:bg-[#111] p-1.5 rounded-full space-x-1 relative w-fit`}>
			{modes.map(mode => (
				<button
					key={mode.themeMode}
